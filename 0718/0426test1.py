...
# 1. change cv
# {{{
for icv in range(3,11):
    print(' cv = ', icv)
    grid_search = GridSearchCV(svr,param_grid, n_jobs=-1, scoring=score, cv=icv)
    grid_search.fit(X_train, y_train)
    print_search_score(grid_search)
    print("learning   score: ",end="")
    print_score(grid_search, X_train, y_train)
    y_pred = grid_search.predict(X_train)
    print('summary',icv, r2_score(y_train,y_pred), grid_search.best_score_)
#   fig = yyplot(y_train,y_pred)
    ...
    scores = cross_val_score(grid_search.best_estimator_,X_train,y_train,scoring=score,cv=icv)
    print(scores)
# }}}
# 2. change cv & shuffle training data 
# {{{
cv = ShuffleSplit(n_splits=10, test_size=0.1, random_state=0)
grid_search = GridSearchCV(svr,param_grid, n_jobs=-1, scoring=score,cv=cv)
grid_search.fit(X_train, y_train)
print_search_score(grid_search)
print("learning   score: ",end="")
print_score(grid_search, X_train, y_train)
y_pred = grid_search.predict(X_train)
print('summary', r2_score(y_train,y_pred), grid_search.best_score_)
# print(grid_search)
# step 3. predict
y_pred = grid_search.predict(X_test)
print_high_Tc(X_test,y_pred)
print("%.2f seconds " % (time() - start))
scores = cross_val_score(grid_search.best_estimator_,X_train,y_train,scoring=score,cv=cv)
np.set_printoptions(precision=2)
print('cross validation, R^2 = ' , scores)
print('%0.3f (+/-%0.03f) for %r' % (scores.mean(), scores.std()*2, grid_search.best_params_))
# }}}
# 3. LeaveOneOut
# {{{
cv = LeaveOneOut()
