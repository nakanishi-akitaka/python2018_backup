...
from my_library              import y_randamization_rgr_scaler, dcv_rgr_scaler
...
niter=10
if(True):
    param_grid2 = {'n_neighbors':range_n}
    scaler = StandardScaler()
    X_train = scaler.fit_transform(X_train)
    dcv_rgr(X_train, y_train, model, param_grid2, niter)
    dcv_rgr_scaler(X_train, y_train, model, scaler, param_grid, niter)
    y_randamization_rgr_scaler(X_train, y_train, model, scaler, param_grid, niter)
    y_randamization_rgr(X_train, y_train, model, param_grid2, niter)
