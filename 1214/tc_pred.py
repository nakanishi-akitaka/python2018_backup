...
1.Hydride Tc Regression (RR, LASSO, EN, k-NN, RF, GB, SVR, PLS, GPR, LWPLS)
...
### Original version
def get_parameters_unused(formula):
    material = Composition(formula)
    features = []
    atomicNo = []
    natom = []
...
range_l = 2**np.arange( -4,  0, dtype=float)
...
range_g = 2**np.arange( -20+15, 11, dtype=float)
...
# range_p = np.arange(1, nprm+2, dtype=int)
...
'LWPLS':{'name':'LWPLS',
         'model':LWPLS(n_components=4),
         'param':{'l_similarity':range_l}},
         }


key = 'LWPLS' # 'RR' 'EN', 'LASSO', 'kNN', 'RF', 'GB', 'SVR', 'PLS', 'GPR', 'LWPLS'
name = model_param[key]['name']
model = model_param[key]['model']
param_grid = model_param[key]['param']
output = 'Tc_' + key + '.csv'
print(name)
print(model)
print(param_grid)
