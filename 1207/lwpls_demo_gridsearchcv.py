# -*- coding: utf-8 -*-
"""
Created on Fri Dec  7 15:25:06 2018
...
for parameter_number, lambda_in_similarity in enumerate(candidates_of_lambda_in_similarity):
    ...
    for fold_number in np.arange(1, number_of_fold_in_cv + 1, 1):
        ...
        lwpls = LWPLS(n_components=r2cvs.shape[0], l_similarity=lambda_in_similarity)
        lwpls.fit(autoscaled_x_train_in_cv, autoscaled_y_train_in_cv)
        estimated_y_validation_in_cv =  lwpls.predict(autoscaled_x_validation_in_cv)
...
optimal_lwpls = LWPLS(n_components=optimal_component_number, l_similarity=optimal_lambda_in_similarity)
optimal_lwpls.fit(autoscaled_x_train, autoscaled_y_train)
estimated_y_test = optimal_lwpls.predict(autoscaled_x_test)
